...
def notify_user(object, action, site):
    ...
    if action == 'rejection':
        ...
        return
    userprofile = UserProfile.objects.get(user=object.recipient)
    if notification:
        ...
        if not DISABLE_USER_EMAILING and user.email and user.is_active and userprofile.email_notification:
